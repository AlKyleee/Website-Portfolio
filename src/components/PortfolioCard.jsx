...
const PortfolioCard = ({ path, image, projectTitle }) => {
    return (
        <Link to={path} className='group flex relative items-center justify-center w-full h-full cursor-pointer bg-black'>
            ...
            <p className="absolute text-[#fff6dc] font-['Ysabeau_SC'] xl:text-5xl sm:text-2xl md:text-7xl lg:text-3xl font-bold group-hover:z-10 text-center">{projectTitle}</p>
        </Link>
    )
}
