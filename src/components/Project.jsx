...
const Project = ({ title, role, year, children, image, link }) => {
    return (
        <motion.div className='flex flex-col h-full w-full py-10 px-10'
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1 }}
        >
            <p className='text-6xl text-portfolio font-semibold mb-10'>{title}</p>
            ...
            <p className="text-xl text-tertiary font-medium text-center self-center w-1/2 mt-12">{children}</p>
            ...
            <img src={image} className='object-cover w-3/5 self-center' /> 
        </motion.div>
    )
}
